...
import { ArrowLeft, Sparkles, ExternalLink, Wand2 } from 'lucide-react';
...
export default function WorkerBrandInspiracionPage({
  params,
}: {
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 28 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 14, color: C.muted, margin: '6px 0 0' }}>
            {brand?.name ?? '…'} · {refs.length} referencias guardadas · {recreations.length} solicitudes de recreación
          </p>
        </div>
      </div>

      {/* Tab switcher referencias / recreaciones */}
          ...
          {filteredRefs.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
              {filteredRefs.map((ref) => (
                <div
                  key={ref.id}
                  style={{ background: C.card, border: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column' }}
                >
                        ...
                        {ref.style_tags.slice(0, 3).map((tag) => (
                          <span
                            ...
                            style={{
                              ...
                              border: `1px solid ${C.border}`, fontFamily: fc, textTransform: 'uppercase', letterSpacing: '0.03em',
                            }}
                          >
                            {tag}
                          </span>
          ...
          {recreations.map((r, i) => {
            ...
            return (
              <div
                ...
                style={{
                  padding: '16px 20px',
                  borderBottom: i < recreations.length - 1 ? `1px solid ${C.border}` : 'none',
                }}
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 8 }}>
                  ...
                  <span style={{
                    fontSize: 10, fontWeight: 800, padding: '3px 8px', color: st.color,
                    background: `${st.color}15`, border: `1px solid ${st.color}44`,
                ...
                {r.client_notes && (
                  <div style={{ fontSize: 13, color: C.text, marginBottom: 6, lineHeight: 1.5 }}>
                    <strong style={{ color: C.muted, fontSize: 11, fontFamily: fc, textTransform: 'uppercase', letterSpacing: '0.05em', marginRight: 8 }}>
                      Cliente:
                    </strong>
                    {r.client_notes}
                  </div>
                ...
                {r.worker_notes && (
                  <div style={{ fontSize: 13, color: C.text, lineHeight: 1.5, paddingTop: 6, borderTop: `1px dashed ${C.border}`, marginTop: 8 }}>
                    <strong style={{ color: C.accent, fontSize: 11, fontFamily: fc, textTransform: 'uppercase', letterSpacing: '0.05em', marginRight: 8 }}>
                      Worker:
                    </strong>
                    {r.worker_notes}
                  </div>
