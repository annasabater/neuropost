...
import type { CopywriterInput, Brand } from '@/types';
...
export async function POST(request: Request) {
  try {
    ...
    const body     = await request.json() as CopywriterInput;
    ...
    const result = await runCopywriterAgent(body, brandToAgentContext(brand as Brand));
    if (!result.success) return NextResponse.json({ error: 'Error al procesar la solicitud' }, { status: 500 });
    return NextResponse.json(result);
  } catch (err) {
    return apiError(err, 'POST /api/agents/copywriter');
  }
}
