...
export async function GET(request: Request) {
  ...
  try {
    ...
    void logSystemAction('cron', 'reconcile_renders', 'system',
      `reconcile-renders: ${retriggered} retriggered, ${permanentlyFailed} permanently failed`,
      { metadata: result });
    return NextResponse.json(result);
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    log({ level: 'error', scope: 'cron/reconcile-renders', event: 'uncaught_error', error: msg });
    Sentry.captureException(err, { tags: { component: 'cron/reconcile-renders' } });
