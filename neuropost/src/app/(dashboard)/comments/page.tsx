    ...
    <div className="page-content">
      <div className="page-header">
        <div className="page-header-text">
          <h1 className="page-title">{t('communityTitle')}</h1>
          <p className="page-sub">{t('communitySubtitle')}</p>
        </div>
      ...
      {summary && (
        <>
          <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: 24 }}>
            <div className="stat-card"><div><p className="stat-label">{t('stats.total')}</p><p className="stat-value">{summary.total}</p></div></div>
            <div className="stat-card"><div><p className="stat-label">{t('stats.autoReplied')}</p><p className="stat-value">{summary.autoResponded}</p></div></div>
            <div className="stat-card"><div><p className="stat-label">{t('stats.escalated')}</p><p className="stat-value">{summary.escalated}</p></div></div>
            <div className="stat-card"><div><p className="stat-label">{t('stats.sentiment')}</p><p className="stat-value">{Math.round(summary.sentimentBreakdown.positive / summary.total * 100)}% +</p></div></div>
          </div>
          {summary.digest && (
            <div className="settings-section" style={{ marginBottom: 24 }}>
              <div className="settings-section-title">{t('aiSummary')}</div>
